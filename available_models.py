...
def get_models(session,action_token):
    ...
    headers = {
        ...
        # has to change when you want image generation
...
def parse_all_model_ids(raw_response: str) -> dict:
    """
    Dynamically extracts Gemini models by recursively searching the JSON structure,
    making it immune to Google shifting array indices.
    """
    model_ids = {}
    ...
    if "Thinking" in raw_response and "Thinking" not in model_ids:
        # We can dynamically regex for the exact 16-char hash associated with thinking logic
        # 5bf011840784117a is universally used for Extended Thinking
...
# MODEL_IDS = parse_all_model_ids(raw_data)
# print("MODEL_IDS =", json.dumps(MODEL_IDS, indent=4))